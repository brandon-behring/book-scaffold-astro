...
- **`provenance` frontmatter object** on all profile schemas (academic / tools / minimal / course-notes / research-portfolio), optional. Fields: `ai_tools[]`, `prompts_archive?`, `decisions_log?`, `audit_history[]` (`{ date, type, file }`), `citation_backstop?`. `audit_history.type` is a free string (real audit types vary); `citation_backstop` is a controlled enum (`research-kb` / `manual` / `unverified`). Path fields use plain `z.string()` — repo-relative paths are not URLs. New exports: `provenanceObject`, `provenanceSchema`, `citationBackstops`, and the inferred `Provenance` type.
...
- **18 new tests** in `package/tests/provenance.test.mjs` (schema parse with/without provenance across all four base schemas, controlled-enum enforcement, relative-path acceptance guarding a `.url()` regression). Total 252 (was 234).
- **Populated exemplar** in `demo/src/content/chapters/v46-seo-demo.mdx` + a README recipe under "What ships in the package".

### Notes

- Visual-regression baselines for the new block are pending regeneration in the CI environment (baselines are environment-bound; a local regen produces environmental-noise diffs across all fixtures).
