 ...
 *
 * The automatic tools chrome (ToolFilter island) must be suppressible
...
test('Base.astro: showChrome gates the automatic ToolFilter island (#163)', () => {
  ...
  // ...and the automatic island renders under showToolsChrome, so the gate applies.
  assert.match(base, /\{\s*showToolsChrome\s*&&\s*<ToolFilter/, 'ToolFilter gated by showToolsChrome');
  assert.doesNotMatch(base, /<VersionSelector\b/, 'VersionSelector is manual opt-in, never Base chrome');
});
