...
Minor release. Adds two static study-guide apparatus components after the v4.17 spine — **`<Diagnostic>`** (#110, the per-chapter pre-reading "Do I Know This Already?" / DIKTA self-check) and **`<PartReview>`** (#111, Part-level interleaved `<Exercise>`-review aggregation). Both static (no new client islands): the scored engine remains the separate #112 increment.
