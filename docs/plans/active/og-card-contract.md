...
**Date:** 2026-07-13 · **Issue:** #157 · **Status:** accepted design; implement next after v5.1.0
...
The feature defaults to `false` in the first release. `exclude` contains
base-relative route patterns, is matched after base normalization, and augments
the built-in exclusions for redirects, error pages, `noindex` pages, and
non-HTML output. Invalid patterns fail config evaluation.
...
2. static book or corpus `seo.ogImage`;
3. generated per-page card.
...
authored image. When neither authored level exists and generation is enabled,
the integration creates the card and adds:
...
The integration runs in `astro:build:done`:
...
```text
<_astro-base>/_og/<first-16-hex-of-content-sha256>.png
```

The hash covers template version, dimensions, preset, book identity, title,
description, and canonical hostname. (The route itself is not drawn on the
card.) Identical visual payloads reuse one file; changed card metadata produces
a new immutable URL. A hash collision with different bytes is a build error.
Only cards referenced by the current build are emitted, so no persistent cache
cleanup contract is required.

## Card content and themes

Every generated card contains:

- page title, clamped to a documented maximum with word-safe truncation;
- optional description, similarly clamped;
- resolved book title (or corpus title for corpus surfaces);
- canonical hostname; and
...
Fonts are loaded from package-owned, versioned local assets. The build performs
no network requests, does not download consumer fonts, and produces the same
bytes for the same payload on supported platforms.
...
include both `site` and normalized Astro `base`; root and subpath deployments
must resolve the exact emitted PNG. The generator never applies a second base
prefix to canonical paths already containing it.
...
- Consumers needing a bespoke card keep using page or static `ogImage`; those
  paths outrank generation.
...
2. page override, static default, generated, and disabled precedence branches
