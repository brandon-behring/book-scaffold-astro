...
- `tests/build-labels.test.mjs` (+4) — the `number` field, the `label=`-override `null`, kind-aware display words sharing one counter (`Theorem 9.1` / `Proposition 9.2` / `Lemma 9.3`), and the fail-loud unknown-kind build failure. `tests/theorem-label.test.mjs` (+1) — the word-only resolution `build-labels` reuses. Two new fixtures (`theorem-kinds.mdx`, `bad-theorem-kind.mdx`). Package suite **385**. The gallery's id'd `<Theorem>` drops its now-redundant `n=` to exercise the `labels.json` path — pixel-neutral, the number still resolves to `4.2`.
