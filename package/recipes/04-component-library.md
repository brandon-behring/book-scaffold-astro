...
The static `/practice-exam` route renders the bank grouped by domain with each answer behind a "Show answer" reveal. `cloze` questions are reserved (schema-accepted, render-deferred). Scoring, the rationale appendix, glossary, and flashcards are later increments — see `docs/plans/active/study-guide-epic_*.md`. (The per-chapter `<Diagnostic>` shipped in v4.19.0, #110.)
